...
<p class = "text"><?php $result_name = mysqli_query($connect, 'SELECT SKILLS_SUMMARY FROM skills_summary'); echo mysqli_fetch_assoc($result_name)['SKILLS_SUMMARY']; ?></p>
  <?php
    $sql_c = 'SELECT DISTINCT SKILL_CATEGORY FROM skills ORDER BY SKILL_CATEGORY';
    $result_c = mysqli_query($connect, $sql_c);
    if(isset($result_c)){
      while($row_c = mysqli_fetch_assoc($result_c)){
  ?>
  <div>
    <h2 style="text-align: center; font-size:xxx-large;"><?php echo $row_c['SKILL_CATEGORY'] ?></h2>  
  </div>
  <div class = "row">
  <?php
    $cat = str_replace("'", "\'", $row_c['SKILL_CATEGORY']);
    $sql_s = 'SELECT * FROM skills WHERE SKILL_CATEGORY = ' . '"' . $cat . '"' . 'ORDER BY SKILL_NAME';
  ...
  <div class = "item">
    <div class = "item-text">
      ...
      <span class = "w-95"><?php echo $row['SKILL_RATIO'] . "%" ?></span>
    </div>
    <div class = "progress">
      <div class = "progress-bar w-<?php echo $row['SKILL_RATIO'] ?>"></div>
    </div>
  </div>
    <?php
      }
